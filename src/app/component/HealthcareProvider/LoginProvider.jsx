...
function LoginProvider() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/api/v1/provider/authenticate', {
        username,
        password,
      });
      const token = response.data;

      localStorage.setItem('providerAuthToken', token);
  ...
  return (
    <div className="login-provider-page">
      <header>
        <Link to="/">
      ...
      <div className="login-container">
        <div className="login-image">
          <img src="/assets/company-log.jpg" alt="Login Illustration" />
        </div>
